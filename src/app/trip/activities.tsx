...
export function Activities({tripDetails}: Props){
    ...
    async function resetNewActivityFields(){
        setActivityTitle("")
        setActivityDate("")
        setActivityHour("")
    ...
    return(
        <View className="flex-1">
                    ...
                    renderSectionHeader={({section}) => (
                        <View className="w-full">
                            ...
                            {section.data.length === 0 && (
                                <Text className="text-zinc-500 font-regular text-sm mb-8">Nenhuma atividade cadastrada nesta data.</Text>
                            )}
                        </View>
                    )}
            ...
            <Modal visible={showModal === MODAL.NEW_ACTIVITY} onClose={() => setShowModal(MODAL.NONE)} title="Cadastrar atividade" subtitle="Todos os convidados podem visualizar as atividades">
                <View className="mt-4 mb-3">
                    <Input variant="secondary">
                        <Tag color={colors.zinc[400]} size={20} />
                        <Input.Field placeholder="Qual atividade?" onChangeText={setActivityTitle} value={activityTitle}></Input.Field>
                    </Input>
                    ...
                    <View className="w-full mt-2 flex-row gap-2">
                        ...
                        <Input variant="secondary" className="flex-1">
                            ...
                            <Input.Field placeholder="Horário?" onChangeText={(text) => setActivityHour(text.replace(".", "").replace(",", ""))} value={activityHour} keyboardType="numeric" maxLength={2}></Input.Field>
                        </Input>
                    </View>
                </View>
            ...
            <Modal title="Selecionar data" subtitle="Selecione a data da atividade" visible={showModal === MODAL.CALENDAR} onClose={() => setShowModal(MODAL.NEW_ACTIVITY)}>
